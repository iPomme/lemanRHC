 ...
 */
  ...
  return {
    ...
    link:function (originalScope, element, attrs, modelCtrl) {
      ...
      /* custom:2/6 */
      var setCaretPos = function(node, offset) {
      ...
      element.bind('keydown', function (evt) {
        /* custom:3/6 */
        var caretPos = window.getSelection ? window.getSelection().getRangeAt(0).startOffset : 0;
        
        // Hide the suggestion box 
        // when the caret reaches one end of a line 
        // and arrow-left/right been pressed
        if (scope.matches.length > 0 && (evt.which === 37 || evt.which == 39)) {
          if ((evt.which === 37 && caretPos === 0) || (evt.which === 39 && caretPos === element.text().length)) {
            resetMatches();
            scope.$digest();
          }
        }
        // Stop update the caret position 
        else if (scope.matches.length === 0 && evt.which === 9) {
          evt.stopImmediatePropagation();
        }
        // Create no new item in the list
        // when the caret is at the beginning of a line
        else if (scope.matches.length === 0 && evt.which === 13) {
          if (caretPos === 0) {
            evt.preventDefault();
            evt.stopImmediatePropagation();
          } 
        ...
        if (evt.which === 40) {
          evt.stopImmediatePropagation() //custom:4/6
          scope.activeIdx = (scope.activeIdx + 1) % scope.matches.length;
          scope.$digest();

        } else if (evt.which === 38) {
          evt.stopImmediatePropagation() //custom:5/6
          ...
          /* custom:6/6 */
          // typeahead didn't upadte the caret position 
          // after a suggestion been selected ..
          evt.preventDefault();
          evt.stopImmediatePropagation();
          
          node = element[0].firstChild;
          offset = element.text().length;
          setCaretPos(node, offset);
          /* end of custom */
      ...
      // Keep reference to click handler to un modelCtrl.$formattersbind it.
